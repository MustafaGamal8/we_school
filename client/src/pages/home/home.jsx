  ...
  return (
    ...
    <>
      ...
      <button onClick={toggleDarkMode} className="h-20  w-10 bg-white drop-shadow-xl rounded-b shadow-2xl shadow-white dark:bg-slate-700   absolute -bottom-20 left-2 ">
        ...
        <div className={` ${isMenuOpen ? "h-max flex md:hidden" : "hidden"} absolute top-[100%] left-0 bg-white text-main  w-full h-0  flex-col-reverse items-center justify-center   whitespace-nowrap dark:bg-slate-700 dark:text-white `}>
      ...
      <div className="w-full bg-white mt-3 flex flex-col md:flex-row items-center justify-around gap-5 p-5 dark:bg-slate-800">
        ...
        <h1 className="text-3xl  text-sec font-semibold animated-title text-center relative w-max m-auto dark:text-white">
          <HiOutlineMail className="inline-block  mr-2 text-2xl md:text-4xl " />
          <Trans>تواصل معنا</Trans>
        </h1>

        <section id="school_contact" className=" lg:w-[50%] w-[80%] mt-16 flex flex-col md:flex-row bg-gray drop-shadow-2xl bg-white rounded-md m-auto justify-between p-2 md:p-10 gap-2 dark:bg-slate-700 dark:text-white" >

          <div className="w-full flex flex-col md:w-[45%]">
            <h1 className="p-2 text-main dark:text-sec text-4xl"><Trans>اتصل بنا</Trans></h1>
            <p className="text-md md:text-2xl p-2"><Trans>نحن هنا من أجلك، كيف يمكننا مساعدتك؟</Trans></p>
            <form onSubmit={handleSubmit} className="mt-5">
              <input type="text" required name="name" placeholder="أدخل اسمك" className="w-full bg-gray-200 placeholder-black outline-none  p-5 rounded-md mt-5 dark:bg-slate-800 dark:placeholder:text-white" />
              <input type="email" required name="email" placeholder="أدخل بريدك الإلكتروني" className="w-full bg-gray-200 placeholder-black outline-none  p-5 rounded-md mt-4 dark:bg-slate-800 dark:placeholder:text-white" />
              <input type="text" required name="message" placeholder="أدخل رسالتك" className="w-full bg-gray-200 placeholder-black outline-none  p-8 rounded-md mt-10 dark:bg-slate-800 dark:placeholder:text-white" />
